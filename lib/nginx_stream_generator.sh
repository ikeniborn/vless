...
# Nginx Configuration Generator (v5.35)
...
#     - Port 4443: MTProxy cloak-port (active probing protection, internal only)
#
# Functions:
...
# ============================================================================
# FUNCTION: generate_nginx_config (v5.35)
# ============================================================================
...
#                 MTProxy cloak-port (port 4443, active probing protection)
# Arguments:
...
#   $6 - enable_mtproxy_cloak: "true"/"false" — include MTProxy cloak-port 4443 (v5.35)
...
generate_nginx_config() {
    ...
    # Debug mode: access_log destination (off by default for privacy)
    local _nginx_access_log="off"
    [[ "${FT_DEBUG:-false}" == "true" ]] && _nginx_access_log="/var/log/nginx/access.log"

    cat <<EOF
# nginx.conf — familytraffic_nginx (v5.35, replaces haproxy.cfg)
...
$(if [[ "${enable_mtproxy_cloak}" == "true" ]]; then
cat <<CLOAK_BLOCK

    # -------------------------------------------------------------------------
    # Port 4443: MTProxy active probing protection (cloak-port, internal only)
    # mtg v2 redirects invalid/probe connections here — censor sees valid TLS
    # with our LE certificate and real HTTP content (not a proxy fingerprint).
    # Do NOT open port 4443 in UFW — this is loopback-only.
    # -------------------------------------------------------------------------
    server {
        listen 4443 ssl;
