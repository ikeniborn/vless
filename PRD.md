...
- **Сетевой режим:** Host network mode (обход проблем Docker NAT)
- **Скрипты:** Bash
- **Базовые утилиты:** qrencode, jq, curl, openssl, ufw (опционально)
- **Дополнительные утилиты для security:** iptables, tcpdump (для диагностики), netstat
...
- Используется network_mode: host для прямого доступа к сети хоста
...
- **xray-server:** Основной контейнер с Xray-core
  ...
  - Порт: 443 (прямое подключение через host network)
  - Network mode: host (обход Docker NAT)
  - Volumes:
...
1. **install.sh** - Мастер установки
   ...
   - Конфигурация firewall:
     ...
     * Fallback на iptables если UFW не установлен
