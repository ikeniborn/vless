...
- **Protocol:** VLESS (lightweight, no built-in encryption)
...
### Container Stack
- **Xray Container:** `teddysun/xray:24.11.30` (VLESS + Reality server)
...
- **Structure:** users.json with UUID, shortId, metadata
...
### Client Support
- **Desktop:** v2rayN (Windows), v2rayNG (Android), Qv2ray (Linux/macOS)
- **Configuration Formats:** JSON, VLESS URI, QR code (PNG/text)
- **Auto-generation:** Client configs created automatically on user addition
...
**Container Layer:** Xray (VLESS + Reality protocol server) and Nginx (fake-site reverse proxy) run in an isolated Docker network (`vless_reality_net`). This isolation ensures zero conflicts with existing VPN services like Outline or Wireguard by maintaining completely separate network namespaces and subnets.
...
| **Xray Container** | Docker Container | VLESS + Reality protocol server, traffic encryption, client authentication | teddysun/xray:24.11.30 | Docker network, config volumes |
...
→ Generate UUID (uuidgen) and shortId (openssl rand 8 hex)
→ Update users.json with new user metadata
→ Update Xray config.json (add to clients[] array)
→ Reload Xray container (docker-compose restart xray)
→ Generate client configurations (JSON, VLESS URI, QR code)
→ Save to /opt/vless/data/clients/username/
→ Display configuration and QR code to administrator
```
...
| ID | Requirement | Priority | Acceptance Criteria | Component |
|----|-------------|----------|---------------------|-----------|
| **FR-005** | **User Creation** | Critical | • User added in < 5 seconds<br/>• UUID uniqueness guaranteed<br/>• ShortId uniqueness among users<br/>• Xray container reloaded without downtime<br/>• Client config saved to data/clients/{username}/ | User Management CLI |
...
3. Read `/opt/vless/config/users.json`
4. Validate username not in use
5. Append new user: `{"username": "...", "uuid": "...", "shortId": "...", "created": "timestamp", "enabled": true}`
6. Write updated users.json atomically
7. Update `/opt/vless/config/config.json` - add to `inbounds[0].settings.clients[]`
8. Reload Xray: `docker-compose restart xray`
9. Generate client configs (see FR-020)
10. Display config and QR code in terminal
...
- `vless-user export <username>`: Regenerate all client config files in data/clients/{username}/
...
**Table 5.1:** Complete non-functional requirements matrix with targets and verification methods.
...
- **User Creation: < 5 seconds**
  - Measurement: Time from `vless-user add username` to QR code display
  - Includes: UUID generation, shortId generation, users.json update, config.json update, Xray reload, client config generation
...
```json
{
  ...
  "inbounds": [{
    ...
    "protocol": "vless",
    "settings": {
      "clients": [{"id": "UUID", "flow": "xtls-rprx-vision", "email": "username"}],
      "decryption": "none",
      "fallbacks": [{"dest": "nginx:8080"}]
    },
    "streamSettings": {
...
{
  "users": [
    {"username": "admin", "uuid": "UUID", "shortId": "shortId", "created": "2025-10-01T12:00:00Z", "enabled": true}
  ],
  "metadata": {"version": "1.0", "last_modified": "2025-10-01T12:00:00Z"}
}
```
...
|---------|------|---------|
| 1.0 | 2025-10-01 | Initial PRD |
| 1.1 | 2025-10-02 | Added sudo accessibility (FR-021) |
| 1.4 | 2025-10-05 | Complete technical architecture |

---

## Conclusion

This PRD defines a comprehensive VLESS + Reality VPN system for 10-50 users, prioritizing ease of use (< 5 min install), security (Reality DPI resistance, X25519 forward secrecy), compatibility (Outline/Wireguard coexistence), and reliability (99.9% uptime).

**Implementation Status:** Ready for development. All 21 functional requirements, 6 NFR categories, and workflows specified with acceptance criteria.
