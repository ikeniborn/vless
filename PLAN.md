...
### ✅ Этап 5: Сервисные функции (ЗАВЕРШЕН)
**Задачи:**
...
**Функции:**
- ✅ `check_service_status()` - статус контейнера и здоровье сервиса
- ✅ `restart_service()` - перезапуск с проверкой здоровья
- ✅ `view_logs()` - вывод логов с опциями --follow и --lines
- ✅ `uninstall_service()` - полное удаление с сохранением скрипта
- ✅ `show_help()` - справочная информация по всем командам

**Результаты:**
- Все сервисные функции реализованы и интегрированы в CLI
- Команда uninstall полностью удаляет сервис с сохранением основного скрипта
- Расширенный просмотр логов с параметрами --follow и --lines
- Команда help предоставляет полную справку по использованию
- Интеграция со всеми существующими функциями управления сервисом
- Корректная обработка ошибок и информативные сообщения
...
```bash
# Системные команды
./vless-manager.sh help          # Справочная информация
./vless-manager.sh install       # Полная установка
./vless-manager.sh uninstall     # Полное удаление сервиса

# Управление сервисом
./vless-manager.sh start         # Запуск сервиса
./vless-manager.sh stop          # Остановка сервиса
./vless-manager.sh restart       # Перезапуск сервиса
./vless-manager.sh status        # Статус сервиса
./vless-manager.sh logs [--follow] [--lines N]  # Просмотр логов

# Управление пользователями
./vless-manager.sh add-user <username>     # Добавление пользователя
./vless-manager.sh remove-user <username>  # Удаление пользователя
./vless-manager.sh list-users              # Список пользователей
./vless-manager.sh show-user <username>    # Детальная информация о пользователе
```
